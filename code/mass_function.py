...
import plotting
import sample
from constants import MASS_FN_PLOTS_DIR, TOTAL_MASS_FUNCTION_KEY


class MassFunction(calculator.CachedCalculator):
    ...
    def total_mass_function(self, rck):
        ...
        self.cache_total_mass_function(rck)
    ...
    def sample_masses(self, rck, radius, existing: unyt.unyt_array = None):
        ...
        # TODO: get from __init__
        sphere_sampler = sample.Sampler(
            self._config, self._dataset_cache, self._cache, self._sim_name)

        sphere_samples = sphere_sampler.sample(rck, radius, z)
